...
struct RichStatsDemo: View {
    ...
    var body: some View {
        DemoSheet {
            VStack(alignment: .leading, spacing: 22) {
                Text("History").couchText(CouchTypography.title)
                ...
                // Completion heat grid — last 5 weeks.
                VStack(alignment: .leading, spacing: 10) {
                    DemoSectionHeader(text: "Last 5 weeks")
                    heatGrid
                }

                // Average vs best per difficulty.
                VStack(alignment: .leading, spacing: 12) {
                    DemoSectionHeader(text: "Average vs. best")
                    bar("Gentle", avg: 0.42, best: 0.30, avgLabel: "3:10", bestLabel: "2:14")
                    bar("Steady", avg: 0.66, best: 0.48, avgLabel: "5:02", bestLabel: "3:40")
                    bar("Sharp", avg: 0.92, best: 0.71, avgLabel: "8:48", bestLabel: "6:31")
                }
    ...
    private func stat(_ v: String, _ l: String) -> some View {
        VStack(spacing: 4) {
            ...
            Text(l).font(.system(size: 11, weight: .medium, design: .rounded)).foregroundStyle(.secondary)
        }
            ...
            ForEach(0..<5, id: \.self) { week in
                HStack(spacing: 5) {
                    ForEach(0..<7, id: \.self) { day in
                        let level = (week * 7 + day * 3) % 4 // 0…3 intensity
                        RoundedRectangle(cornerRadius: 5, style: .continuous)
                            .fill(accent.opacity([0.08, 0.35, 0.6, 0.95][level]))
                            .frame(height: 22)
                    }
                }
            }
    ...
    private func bar(_ title: String, avg: Double, best: Double, avgLabel: String, bestLabel: String) -> some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            GeometryReader { geo in
                ZStack(alignment: .leading) {
                    ...
                    Capsule().fill(accent.opacity(0.35)).frame(width: geo.size.width * avg, height: 8)
                    Capsule().fill(accent).frame(width: geo.size.width * best, height: 8)
                }
            }
            .frame(height: 8)
        }
...
/// A tiny filled sparkline for the solve-time trend.
private struct Sparkline: View {
    let points: [Double]  // 0 (fast) … 1 (slow), drawn top-down inverted
    let accent: Color
    var body: some View {
        GeometryReader { geo in
