...
COMMENTS = {
    ...
    "board.rose.eraseDigit": "A VoiceOver custom action, and the label of a rose petal whose digit is already placed in this square (or already pencilled in, in pencil mode). %1$lld is the digit 1-9. A verb in the imperative, matching board.action.place/board.action.note — tapping the petal clears the digit rather than re-placing it. Says only that the digit is here, never that it is wrong: do not translate this as if it were a warning.",
